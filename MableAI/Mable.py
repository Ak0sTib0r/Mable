        ...
        with sr.Microphone() as source:
            ...
            for i in range(0, len(inputText)):
            	...
            	if inputText[i] == "my" and inputText[i+1] == "name" and inputText[i+2] == "is":
            		engine.say("Nice to meet you," + inputText[i+3])
            		engine.runAndWait()

            	if inputText[i] == "your" and inputText[i+1] == "name":
            		engine.say("My name is " + AIname)
            		engine.say("What's yours?")
            		engine.runAndWait()
            		functionIndex = "S"

            	if inputText[i] == "joke":
            		engine.say("No. I'm not a comedian, screw you human.")
            		engine.runAndWait()
            		functionIndex = "S"
            ...
            if inputText[0] == "tell" and inputText[1] == "me" and inputText[2] == "about":
            	MyText = MyText.replace("tell", "")
            	MyText = MyText.replace("me", "")
            	MyText = MyText.replace("about", "")
            	engine.say("Okay, here's what I found on " + MyText)
            	engine.runAndWait()
            	webbrowser.get('C:/Program Files/Google/Chrome/Application/chrome.exe %s').open("https://en.wikipedia.org/wiki/"+MyText)
